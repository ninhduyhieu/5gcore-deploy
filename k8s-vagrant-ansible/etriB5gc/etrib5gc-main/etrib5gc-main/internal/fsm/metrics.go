...
func newFsmMetrics() FsmMetrics {
    return FsmMetrics{evMetrics: make(map[EventType]*EventMetrics)}
}

func (m *FsmMetrics) onTriggered() {
    m.mutex.Lock(); m.triggered++; m.mutex.Unlock()
}

func (m *FsmMetrics) onSubmitted() {
    m.mutex.Lock(); m.submitted++; m.mutex.Unlock()
}
...
func (m *FsmMetrics) onCompleted(evType EventType, started time.Time) {
    ...
    ev.add(time.Since(started).Nanoseconds())
    m.mutex.Unlock()
}
...
func (m *FsmMetrics) getInfo() *FsmInfo {
    ...
    info := &FsmInfo{NumSubmitted: m.submitted, NumTriggered: m.triggered, NumCompleted: m.completed}
    info.EvStats = make([]EventInfo, len(m.evMetrics))
    i := 0
    for evType, stats := range m.evMetrics {
        info.EvStats[i] = EventInfo{
            EvType: int(evType), Count: stats.cnt, ResetCount: stats.resetCnt, Duration: stats.sumDuration,
        }
        i++
    }
    return info
...
func (m *EventMetrics) add(duration int64) {
    if m.cnt == math.MaxUint32 {
        m.cnt, m.sumDuration, m.resetCnt = 0, 0, m.resetCnt+1
    }
    m.cnt++; m.sumDuration += duration
}
