...
/*
func IPAddressToString(ipAddr ies.TransportLayerAddress) (ipv4Addr, ipv6Addr string) {
		ip := ipAddr.Value
...
func IPAddressToNgap(ipv4Addr, ipv6Addr string) (ipAddr ies.TransportLayerAddress, err error) {
		if len(ipv4Addr) == 0 && len(ipv6Addr) == 0 {
			err = fmt.Errorf("IPAddressToNgap: Both ipv4 & ipv6 are nil string")
			return
		}

		if len(ipv4Addr) > 0 && len(ipv6Addr) > 0 { // Both ipv4 & ipv6
